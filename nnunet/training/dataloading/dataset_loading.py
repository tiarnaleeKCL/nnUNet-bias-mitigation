...
class DataLoader2D_oversampling(SlimDataLoaderBase):
    def __init__(self, data, patch_size, final_patch_size, batch_size, oversample_foreground_percent=0.0,
                 ...
                 pad_kwargs_data=None, pad_sides=None, folder = None):
        """
        ...
        """
        super(DataLoader2D_oversampling, self).__init__(data, batch_size, None)
        if pad_kwargs_data is None:
            pad_kwargs_data = OrderedDict()
        self.pad_kwargs_data = pad_kwargs_data
        self.pad_mode = pad_mode
        ...
        self.folder_with_preprocessed_data = folder
    ...
    def generate_train_batch(self):
        ...
        #The below code should be uncommented to use the stratified batch sampling
        
        task_name = self.folder_with_preprocessed_data.split('/')[-2]
        
        data_csv = pd.read_csv(/path/to/your/raw_data_base/+ task_name +'/labels.csv', header=None).values

        
        race_labels = np.array([data_csv[i, 1] for i in range(len(data_csv)) if data_csv[i, 0] in self._data])
        cases_s = [data_csv[i, 0] for i in range(len(data_csv)) if data_csv[i, 0] in self._data]
        
        # Find number and keys of each class
        included_labels, num_classes = np.unique(race_labels, return_counts=True)
        
        # Ensure race_labels is an array before using it for indexing
        cases_class_0_indices = np.where(race_labels == included_labels[0])[0]
        cases_class_1_indices = np.where(race_labels == included_labels[1])[0]
        
        # Now you can index cases_s with the indices
        cases_class_0 = [cases_s[i] for i in cases_class_0_indices]
        cases_class_1 = [cases_s[i] for i in cases_class_1_indices]

        
        if num_classes[0] != num_classes[1]:
                if len(cases_class_0) < len(cases_class_1):
               
                    # Oversample class 0 if needed
                    selected_keys_class = np.random.choice(cases_class_0, round(self.batch_size/2), True, None) #ptp finds max - min
                    selected_keys_class1 = np.random.choice(cases_class_1, round(self.batch_size/2), True, None)
                    selected_keys  = list(np.hstack((selected_keys_class, selected_keys_class1)))
                    
                elif len(cases_class_0) > len(cases_class_1):
               
                    # Oversample class 1 if needed
                    selected_keys_class = np.random.choice(cases_class_1, round(self.batch_size/2), True, None)
                    selected_keys_class0 = np.random.choice(cases_class_0, round(self.batch_size/2), True, None)
                    selected_keys  = list(np.hstack((selected_keys_class, selected_keys_class0)))
