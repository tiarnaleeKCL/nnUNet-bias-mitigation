...
from nnunet.training.dataloading.dataset_loading import load_dataset, DataLoader3D, DataLoader2D, unpack_dataset, DataLoader2D_oversampling
from nnunet.training.loss_functions.dice_loss import DC_and_CE_loss, DC_and_CE_loss_with_reweighting, GroupDRO_with_CE, GroupDRO_with_CE_reweighting
...
class nnUNetTrainer(NetworkTrainer):
    def __init__(self, plans_file, fold, output_folder=None, dataset_directory=None, batch_dice=True, stage=None,
        ...
        self.extra_label = extra_label #new
    
        # self.loss = DC_and_CE_loss_with_reweighting({'batch_dice': self.batch_dice, 'smooth': 1e-5, 'do_bg': False}, {}, extra_label = self.extra_label)
        # self.loss = GroupDRO_with_CE({'batch_dice': self.batch_dice, 'smooth': 1e-5, 'do_bg': False}, {})
        # self.loss = GroupDRO_with_CE_reweighting({'batch_dice': self.batch_dice, 'smooth': 1e-5, 'do_bg': False}, {})
    ...
    def get_basic_generators(self):
        ...
        if self.threeD:
            ...
            dl_tr = DataLoader2D(self.dataset_tr, self.basic_generator_patch_size, self.patch_size, self.batch_size,
                                  oversample_foreground_percent=self.oversample_foreground_percent,
                                  pad_mode="constant", pad_sides=self.pad_all_sides, memmap_mode='r', folder = self.folder_with_preprocessed_data)
            dl_val = DataLoader2D(self.dataset_val, self.patch_size, self.patch_size, self.batch_size,
                                  oversample_foreground_percent=self.oversample_foreground_percent,
                                  pad_mode="constant", pad_sides=self.pad_all_sides, memmap_mode='r', folder = self.folder_with_preprocessed_data)
            
            # dl_tr = DataLoader2D_oversampling(self.dataset_tr, self.basic_generator_patch_size, self.patch_size, self.batch_size,
            #                      oversample_foreground_percent=self.oversample_foreground_percent,
            #                      pad_mode="constant", pad_sides=self.pad_all_sides, memmap_mode='r', folder = self.folder_with_preprocessed_data)
            # dl_val = DataLoader2D_oversampling(self.dataset_val, self.patch_size, self.patch_size, self.batch_size,
            #                       oversample_foreground_percent=self.oversample_foreground_percent,
            #                       pad_mode="constant", pad_sides=self.pad_all_sides, memmap_mode='r', folder = self.folder_with_preprocessed_data)
