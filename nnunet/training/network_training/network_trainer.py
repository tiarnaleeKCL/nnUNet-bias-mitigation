...
class NetworkTrainer(object):
    ...
    def plot_progress(self):
        ...
        try:
            ...
            ax.plot(x_values, self.all_tr_losses, color='b', ls='-', label="loss_tr", linewidth=5)

            ax.plot(x_values, self.all_val_losses, color='r', ls='-', label="loss_val, train=False", linewidth=5)

            if len(self.all_val_losses_tr_mode) > 0:
                ax.plot(x_values, self.all_val_losses_tr_mode, color='g', ls='-', label="loss_val, train=True", linewidth=5)
            if len(self.all_val_eval_metrics) == len(x_values):
                ax2.plot(x_values, self.all_val_eval_metrics, color='g', ls='--', label="evaluation metric", linewidth=5)
